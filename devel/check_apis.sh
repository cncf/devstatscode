...
curl -s -H 'Content-Type: application/json' https://teststats.cncf.io/api/v1 -d'{"api": "DevActCnt", "payload": {"project": "cncf", "range": "Last century", "metric": "contributions", "repository_group": "All", "country": "All", "github_id": "lukaszgryglicki"}}' | jq -r '.'
curl -s -H 'Content-Type: application/json' https://teststats.cncf.io/api/v1 -d'{"api":"CumulativeCounts","payload":{"project":"cncf","metric":"contributors"}}' | jq -r '.'
curl -s -H 'Content-Type: application/json' https://teststats.cncf.io/api/v1 -d'{"api":"CumulativeCounts","payload":{"project":"cncf","metric":"organizations"}}' | jq -r '.'
