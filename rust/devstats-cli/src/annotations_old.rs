...
async fn write_ts_points_to_db(
    ...
    for (table_name, table_points) in series_tables {
        ...
        if !table_exists(pool, &table_name).await? {
            create_ts_table(pool, &table_name, table_points[0]).await?;
        }

        // Insert points
        for point in table_points {
            insert_ts_point(pool, &table_name, point, ctx).await?;
        }
    }
...
/// Create time series table - replicates Go's table creation logic
async fn create_ts_table(pool: &PgPool, table_name: &str, sample_point: &TSPoint) -> Result<()> {
    let mut columns = vec![
        "time timestamp not null".to_string(),
        "time_hour timestamp not null".to_string(),
    ];

    // Add columns based on tags
    if let Some(ref tags) = sample_point.tags {
    ...
    if let Some(ref fields) = sample_point.fields {
    ...
    let index_sql = format!(
        "CREATE INDEX \"{}\" ON \"{}\" USING btree (\"time\")",
        format!("{}_time_idx", table_name),
        table_name
    );
...
/// Insert single TSPoint into table
async fn insert_ts_point(
    pool: &PgPool,
    table_name: &str,
    point: &TSPoint,
    ctx: &Context,
) -> Result<()> {
    let mut columns = vec!["time", "time_hour"];
    let mut values = vec!["$1".to_string(), "$2".to_string()];
    let mut bind_values: Vec<Box<dyn sqlx::Encode<'_, sqlx::Postgres> + Send>> = vec![
        Box::new(point.time),
        Box::new(hour_start(point.time)),
    ];
    let mut bind_idx = 3;

    // Add tag columns and values
    if let Some(ref tags) = point.tags {
        for (key, value) in tags {
            columns.push(key);
            values.push(format!("${}", bind_idx));
            bind_values.push(Box::new(value.clone()));
            bind_idx += 1;
        }
    }

    // Add field columns and values  
    if let Some(ref fields) = point.fields {
        for (key, value) in fields {
            columns.push(key);
            values.push(format!("${}", bind_idx));
            match value {
                serde_json::Value::String(s) => bind_values.push(Box::new(s.clone())),
                serde_json::Value::Number(n) => {
                ...
                serde_json::Value::Bool(b) => bind_values.push(Box::new(*b)),
                _ => bind_values.push(Box::new(value.to_string())),
            }
            bind_idx += 1;
        }
    }
    ...
    // Execute the query - this is complex due to Rust's type system
    // For now, use a simpler approach with pre-formatted values
    let formatted_sql = format_insert_sql(table_name, point);
    sqlx::query(&formatted_sql).execute(pool).await?;

    Ok(())
}

/// Format insert SQL with actual values (simpler approach)
fn format_insert_sql(table_name: &str, point: &TSPoint) -> String {
    let mut columns = vec!["time", "time_hour"];
    let mut values = vec![
        format!("'{}'", point.time.format("%Y-%m-%d %H:%M:%S")),
        format!("'{}'", hour_start(point.time).format("%Y-%m-%d %H:%M:%S")),
    ];
