...
async fn main() -> Result<()> {
    ...
    if ctx.project.is_empty() {
        error!("You need to set project via GHA2DB_PROJECT environment variable");
        std::process::exit(1);
    }

    info!("GitHub API to DB sync starting for project: {}", ctx.project);
    ...
    match check_rate_limits(&client).await {
        Ok((remaining, reset_time)) => {
            info!("GitHub API rate limit: {} remaining, resets at {}", 
                remaining, reset_time.format("%Y-%m-%d %H:%M:%S"));
            
            if remaining < 100 {
                error!("GitHub API rate limit too low: {} remaining", remaining);
                std::process::exit(1);
            }
        }
        Err(err) => {
    ...
    let mut processed_issues = 0u32;
    let mut processed_prs = 0u32;
...
async fn process_repository_issues(
    ...
    Ok((rng.gen_range(0..10), rng.gen_range(0..5)))
}
