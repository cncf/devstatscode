//! Git commit-message trailers (Go `trailers.go`): the trailer line
//! pattern and the map of accepted trailer names → canonical role names,
//! shared by `gha2db` and `get_repos`.

use regex::Regex;
use std::collections::HashMap;
use std::sync::LazyLock;

/// Go `GitTrailerPattern`: `^(?P<name>[a-zA-Z0-9\-]+)\:[ \t]+(?P<value>.+)$`.
pub static GIT_TRAILER_PATTERN: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^(?P<name>[a-zA-Z0-9\-]+):[ \t]+(?P<value>.+)$").unwrap());

/// Go `GitAllowedTrailers` as a sorted (lower-case trailer name, roles) table.
pub const GIT_ALLOWED_TRAILERS_TABLE: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &["Co-authored-by", "Reported-by", "Tested-by"],
    ),
...
/// Go `GitAllowedTrailers`: lower-case trailer name → canonical role names.
pub static GIT_ALLOWED_TRAILERS: LazyLock<HashMap<&'static str, &'static [&'static str]>> =
    LazyLock::new(|| GIT_ALLOWED_TRAILERS_TABLE.iter().copied().collect());

/// One parsed trailer role (Go `trailer` in `cmd/get_repos/fetch_commits.go`).
...
/// Go `parseTrailers` (without its debug line): the roles of the message's
/// trailer lines — `Name: Value` lines whose lower-case name is an accepted
/// trailer and whose value is `Name <email>` (both parts non-empty).
pub fn parse_trailers(msg: &str) -> Vec<Trailer> {
    ...
    for l in msg.split('\n') {
        ...
        let Some(caps) = GIT_TRAILER_PATTERN.captures(l) else {
            continue;
        };
        let key = caps["name"].trim().to_lowercase();
        let value = caps["value"].trim();
        let Some(roles) = GIT_ALLOWED_TRAILERS.get(key.as_str()) else {
            continue;
        };
        if roles.is_empty() {
            continue;
        }
        // Go: strings.Split(value, "<")[1] then strings.Split(that, ">")[0].
        let name_email: Vec<&str> = value.split('<').collect();
        if name_email.len() < 2 {
            continue;
        }
        let name = name_email[0].trim();
        let email_end: Vec<&str> = name_email[1].split('>').collect();
        if email_end.len() < 2 {
            continue;
        }
        let email = email_end[0].trim();
...
mod tests {
    ...
    fn table_is_complete_and_unique() {
        assert_eq!(GIT_ALLOWED_TRAILERS_TABLE.len(), 348);
        assert_eq!(GIT_ALLOWED_TRAILERS.len(), 348);
        assert_eq!(GIT_ALLOWED_TRAILERS["signed-off-by"], &["Signed-off-by"]);
        assert_eq!(
            GIT_ALLOWED_TRAILERS["thanks-to"],
            &["Influenced-by", "Informed-by"]
        );
        // Go keeps a key with a trailing space — it can never match a trimmed name.
        assert!(GIT_ALLOWED_TRAILERS.contains_key("badly-reviewed-by "));
    }

    #[test]
    fn parse_trailers_follows_go() {
        let msg = "Subject\n\nSigned-off-by: Alice <alice@example.com>\r\nReviewed-By:\tBob Smith <bob@example.com>\nTested-and-reported-by: Carol <carol@x>\nAcked-by: nobody\nCo-authored-by: <only@email>\nSigned-off-by: Name <>\nUnknown-trailer: X <y@z>\nSigned-off-by:no space <a@b>\n";
        let got = parse_trailers(msg);
        let want = vec![
            Trailer {
                role: "Signed-off-by".into(),
                name: "Alice".into(),
        ...
        // Value with several '<': Go splits on every '<' and uses the second part.
        let got = parse_trailers("Signed-off-by: A <b@c> <d@e>");
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].email, "b@c");
        // Missing '>' after the email is not a trailer.
        assert!(parse_trailers("Signed-off-by: A <b@c").is_empty());
        // A second '<' before the '>' cuts the email part (Go Split semantics).
        assert!(parse_trailers("Signed-off-by: A <b<c@d>").is_empty());
        assert_eq!(parse_trailers("Signed-off-by: A <b@c<x>").len(), 0);
        assert_eq!(parse_trailers("Signed-off-by: A <b@c>x<y>")[0].email, "b@c");
    }
}
